...
* **Streamlit in Snowflake アプリケーション**（4つのページで構成）：
  * **サプライチェーンアシスタント** — Cortex Agents API によるマルチターン会話チャット（サプライチェーン + 天気予報のデュアルセマンティックモデル対応、ドリルダウン分析、SQLクエリ結果のインライン表示）
  * **定点観測ダッシュボード** — 事業ラインフィルタ付きのKPIメトリクス、注文分析、在庫アラート一覧、工場概要の可視化
  * **最適化実行** — SciPy 線形計画法による工場間材料移送のコスト最適化
...
## ステップ 6 - Streamlit アプリケーションのデプロイ

Snowflake Intelligence に加え、Streamlit in Snowflake（SiS）アプリケーションとしても同等の機能を利用できます。このアプリでは Cortex Agents API を直接呼び出し、Snowflake Intelligence を開かずにデータ分析やドリルダウンが可能です。

### アプリの作成

1. Snowsight で **Streamlit** に移動し、**+ Streamlit App** をクリック
2. データベース `SUPPLY_CHAIN_ASSISTANT_DB`、スキーマ `ENTITIES`、ウェアハウス `SUPPLY_CHAIN_ASSISTANT_WH` を選択
3. `streamlit/supply_chain_assistant_app.py` の内容をエディタに貼り付け
4. `streamlit/cortex_image.png` をアプリのステージにアップロード
5. パッケージ設定で `streamlit/environment.yml` の内容を反映（`scipy` パッケージが必要）
...
* **Cortex Agents API** によるマルチターン会話 — 前の回答を踏まえた追加質問やドリルダウンが可能
* **デュアルセマンティックモデル** — サプライチェーンデータと天気予報データの両方に対応
* **Cortex Search** — サプライチェーンドキュメントの検索
* **インライン結果表示** — SQLクエリ結果をデータフレームとして直接表示、SQL文はエキスパンダーで確認可能
* **引用表示** — Cortex Search の検索結果ソースを表示
* **サジェスチョンボタン** — カテゴリ別のサンプル質問をワンクリックで実行
* **会話リセット** — サイドバーからチャット履歴をクリア

#### 📊 定点観測ダッシュボード

* **事業ラインフィルタ** — マルチセレクトで特定の事業ラインに絞り込み
* **KPI メトリクス** — アクティブ注文数、総注文金額、製造工場数、在庫アラート数を一覧表示
* **注文分析チャート** — 事業ライン別の注文金額を棒グラフで可視化
* **在庫アラート一覧** — 最低在庫レベルを下回る材料の詳細テーブル
* **工場概要** — 各製造工場の所在地・ステータス・事業ラインを一覧表示

#### 🚀 最適化実行

* **SciPy 線形計画法**（HiGHS ソルバー）による工場間材料移送の最適化
* 余剰在庫のある工場から不足工場への最適な移送計画と新規購入コストとの比較

### environment.yml について

Streamlit in Snowflake でアプリを動作させるには、以下のパッケージが必要です：
...
> **注意：** `scipy` はバージョンを指定せずにインストールしてください。最適化機能で使用されます。

## 次のステップ

おめでとうございます！以下を組み合わせた、Snowflake Intelligence および Streamlit in Snowflake を活用した包括的なサプライチェーンアシスタントの構築が完了しました：

* **デュアル分析**：サプライチェーン運用と天候データの両方をクエリ
* **セマンティック検索**：非構造化サプライチェーンドキュメントへのアクセス
* **Web 統合**：外部情報の検索とスクレイピング（Snowflake Intelligence 経由）
* **コミュニケーション**：HTML ニュースレターの生成とメール送信（適切な統合が必要）
* **Streamlit アプリ**：Cortex Agents API によるインタラクティブなチャット分析、定点観測ダッシュボード、在庫移送最適化
