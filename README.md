...
現代のサプライチェーン運用は、複数の製造拠点にわたる原材料在庫の効率的な管理という重大な課題に直面しています。運用管理者は常に在庫レベルのバランスを取り、余剰と不足のある工場間で材料を移送するか、サプライヤーから新たに購入するかを判断しなければなりません。これらの意思決定を手動で行うことは、時間がかかり、エラーが発生しやすく、コスト面で最適でない結果をもたらすことが多いです。
...
3. **SUPPLY_CHAIN_ASSISTANT_PDF_STAGE** ステージで、右上の **+ Files** ボタンを使用して **pdfs/Supply Chain Network Overview.pdf** ファイルをアップロード
...
* "先月、注文はいくつ受けましたか？"
* "どの製造工場で、どの原材料の在庫が少ないですか？"
* "注文金額上位5社の顧客は誰ですか？"
* "製造工場にある完成品の総数量はいくつですか？"
* "原材料の在庫が少ない製造工場はどこで、同じ材料の余剰在庫がある工場はどこですか？"
* "原材料の在庫が少ない工場について、サプライヤーから補充するコストと余剰在庫のある工場から移送するコストを比較してください"
